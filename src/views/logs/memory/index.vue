<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="created_at" min-width="150px">
        <template slot-scope="{row}">
          <span>{{ row.created_at }}</span>
        </template>
      </el-table-column>
      <el-table-column label="memory_end_at" min-width="150px">
        <template slot-scope="{row}">
          <span>{{ row.memory_end_at }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="Detail" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="35%" style="width: 80%; margin-left:50px;">
        ...
        <el-form-item label="memory_at" prop="end_point">
          <el-input v-model="temp.memory_at" />
        </el-form-item>
        <el-form-item label="operation" prop="merchant">
          <el-input v-model="temp.operation" />
        </el-form-item>
        <el-form-item label="created_at" prop="method">
          <el-input v-model="temp.created_at" />
        </el-form-item>
        <el-form-item label="memory_end_at" prop="total_request">
          <el-input v-model="temp.memory_end_at" />
        </el-form-item>
        <el-form-item label="job_id" prop="created_at">
...
<script>
  import { listMemory } from '@/api/logs'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        listMemory(this.listQuery).then(response => {
